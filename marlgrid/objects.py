...
class WorldObj(metaclass=RegisteredObjectType):
    def __init__(self, color="worst", state=0, type=""):
        self.color = color
        self.state = state
        self.contains = None
...
class GridAgent(WorldObj):
    def __init__(self, *args, color='red', **kwargs):
        ...
        self.is_agent = True
        self.atype = kwargs["type"]
        if kwargs["type"] and kwargs["type"] == "teacher":
            self.pos_init = [1, 1]
            self.set_position(self.pos_init)
        if kwargs["type"] and kwargs["type"] == "student":
            self.pos_init = [3, 2]
            self.set_position(self.pos_init)
