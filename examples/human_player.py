...
while not done:
    ...
    player1_action = human.action_step(obs_list[0]['pov'])
    player2_action = human.action_step(obs_list[0]['pov'])
    # The environment expects a list of actions, so put the player action into a list
    agent_actions = [player1_action, player2_action]

    next_obs_list, rew_list, done, _ = env.step(agent_actions)

    human.save_step(
        obs_list[0], player1_action, rew_list[0], done
    )
